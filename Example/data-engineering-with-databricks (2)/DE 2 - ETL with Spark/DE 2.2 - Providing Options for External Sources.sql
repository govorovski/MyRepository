...
-- MAGIC %python
-- MAGIC people = spark.createDataFrame([
-- MAGIC     {"deptId": 1, "age": 40, "name": "Hyukjin Kwon", "gender": "M", "salary": 50},
-- MAGIC     {"deptId": 1, "age": 50, "name": "Takuya Ueshin", "gender": "M", "salary": 100},
-- MAGIC     {"deptId": 2, "age": 60, "name": "Xinrong Meng", "gender": "F", "salary": 150},
-- MAGIC     {"deptId": 3, "age": 20, "name": "Haejoon Lee", "gender": "M", "salary": 200}
-- MAGIC ])
-- MAGIC people.show()
...
-- MAGIC %python
-- MAGIC
-- MAGIC #print(spark.range(1).schema)
-- MAGIC
-- MAGIC datafile=spark.read.schema("order_id LONG, email STRING, transactions_timestamp LONG, total_item_quantity INTEGER, purchase_revenue_in_usd DOUBLE, unique_items INTEGER, items STRING").option("delimiter", "|").option("header", True).csv(DA.paths.sales_csv,header=True)
-- MAGIC print(datafile.schema)
-- MAGIC datafile.show(5)
-- MAGIC datafile.write.saveAsTable("sales_csv")

-- COMMAND ----------

-- MAGIC   %python
-- MAGIC   import pyspark.sql.types as T
-- MAGIC _ = spark.sql("DROP TABLE sales_csv")
-- MAGIC df = spark.catalog.createTable("sales_csv", schema=T.StructType([
-- MAGIC     T.StructField("order_id", T.LongType()),
-- MAGIC     T.StructField("email", T.StringType()),
-- MAGIC     T.StructField("transactions_timestamp", T.LongType()),
-- MAGIC     T.StructField("total_item_quantity", T.IntegerType()),
-- MAGIC     T.StructField("purchase_revenue_in_usd", T.DoubleType()),
-- MAGIC     T.StructField("unique_items", T.IntegerType()),
-- MAGIC     T.StructField("items", T.StringType())
-- MAGIC ]),  path=DA.paths.sales_csv, source='csv', options="delimiter,|")
-- MAGIC df.show()
-- MAGIC
...
-- MAGIC %python
-- MAGIC df = spark.read.option("delimiter", "|").option("header", True).csv(DA.paths.sales_csv)
-- MAGIC df.show()
