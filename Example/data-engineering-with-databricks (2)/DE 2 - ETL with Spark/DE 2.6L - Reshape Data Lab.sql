...
event_name, count(*)
from events
where user_id='UA000000105168174'
...
delivery,
premium
from (select user_id as user, event_name, count(event_name) as cnt
       from events
      group by user_id, event_name
     )
PIVOT (
  max(cnt) FOR event_name IN (
'cart',
'pillows',
'login',
'main',
...
-- MAGIC
-- MAGIC eventsDF = spark.table("events")
-- MAGIC #events_pivotDF = eventsDF.selectExpr("user_id", "event_name").groupBy("user_id", "event_name").agg(count("event_name").alias("cnt"))
-- MAGIC events_pivotDF = eventsDF.selectExpr("user_id", "event_name").groupBy("user_id", "event_name").agg(count("event_name").alias("cnt")).groupBy("user_id").pivot("event_name").max("cnt").select(col("user_id").alias("user"),
-- MAGIC "cart",
-- MAGIC "pillows",
-- MAGIC "login",
-- MAGIC "main",
...
-- MAGIC "premium")
-- MAGIC
-- MAGIC #.where(col("user_id") == 'UA000000105168174')
-- MAGIC events_pivotDF.createOrReplaceTempView("events_pivot")
-- MAGIC
-- MAGIC events_pivotDF2 = spark.table("events_pivot")
-- MAGIC display(events_pivotDF2)
-- MAGIC
-- MAGIC
-- MAGIC #events_pivotDF)
-- MAGIC #(spark.read
-- MAGIC #    <FILL_IN>
...
-- MAGIC %python
-- MAGIC #events_pivotDF2.printSchema()
-- MAGIC #events_pivotDF2.columns
-- MAGIC #events_pivotDF2.describe().show()
-- MAGIC events_pivotDF2.show()
...
-- MAGIC %python
-- MAGIC # TODO
-- MAGIC eventsPivotDF = spark.read.table("events_pivot")
-- MAGIC transactionsDF = spark.read.table("transactions")
-- MAGIC clickpathsDF = eventsPivotDF.join(transactionsDF, eventsPivotDF.user == transactionsDF.user_id)
