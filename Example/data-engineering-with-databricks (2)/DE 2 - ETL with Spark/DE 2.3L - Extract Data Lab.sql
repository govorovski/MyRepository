...
-- MAGIC wd = spark.table("events_json").withColumn("ffset", col("offset")).writeStream.outputMode("complete").table("new_events_json")
-- MAGIC
-- MAGIC total = spark.table("new_events_json").count()
-- MAGIC
-- MAGIC print(f"Count of events_json = {total}")
-- MAGIC #assert total == 2251, f"Expected 2252 records, found {total}"
