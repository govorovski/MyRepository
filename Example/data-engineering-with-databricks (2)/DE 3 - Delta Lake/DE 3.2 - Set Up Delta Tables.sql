...
-- MAGIC %python
-- MAGIC f = dbutils.fs.ls(f"{DA.paths.datasets}/ecommerce/raw/sales-csv")
-- MAGIC display(f)
-- MAGIC
...
update purchases set transaction_timestamp=1000
where id = 257436;
...
-- MAGIC tbl_location = spark.sql(f"DESCRIBE DETAIL purchase_dates").first().location
-- MAGIC print(tbl_location)
-- MAGIC
-- MAGIC files = dbutils.fs.ls(tbl_location)
-- MAGIC #display(files)
-- MAGIC
-- MAGIC #display(dbutils.fs.ls(f"{DA.paths.user_db}/purchase_dates/_delta_log"))
-- MAGIC display(spark.sql(f"SELECT * FROM json.`{DA.paths.user_db}/purchase_dates/_delta_log/00000000000000000002.json`"))
-- MAGIC region ="nyc"
-- MAGIC store = "100"
-- MAGIC year = "2021"
-- MAGIC
-- MAGIC
-- MAGIC #df = spark.sql(f"CREATE OR REPLACE TABLE {region}{store}_sales_{year} (id INT)")
-- MAGIC #display(df)
-- MAGIC
-- MAGIC #df = spark.read.format("delta").load

-- COMMAND ----------

select* from nyc100_sales_2021;
